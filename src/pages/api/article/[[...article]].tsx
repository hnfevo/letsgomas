...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    ...
    if (id) {
      ...
      if (article) {
        res.status(200).json({ status: true, statusCode: 200, message: 'success', data: article });
      } else {
        res.status(404).json({ status: false, statusCode: 404, message: 'Data not found' });
      }
    } else {
      const articles = await retriveData('article');
      res.status(200).json({ status: true, statusCode: 200, message: 'success', data: articles });
    }
    ...
    jwt.verify(token, process.env.NEXTAUTH_SECRET || ' ', async (err: any, decoded: any) => {
      if (decoded && decoded.role === 'admin') {
        await updateData('article', article[1], data, (result: boolean) => {
          if (result) {
            res.status(200).json({
              status: true,
              statusCode: 200,
    ...
    jwt.verify(token, process.env.NEXTAUTH_SECRET || ' ', async (err: any, decoded: any) => {
      if (decoded && decoded.role === 'admin') {
        await deleteData('article', article[1], (result: boolean) => {
          if (result) {
            res.status(200).json({
              status: true,
              statusCode: 200,
